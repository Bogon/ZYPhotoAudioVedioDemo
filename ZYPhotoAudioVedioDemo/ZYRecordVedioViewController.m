...
@interface ZYRecordVedioViewController ()<AVCaptureFileOutputRecordingDelegate>//视频文件输出代理
{
    NSTimer *timer;
    NSInteger timeCount;
    NSURL *vedioSavePath;
...
@property (nonatomic, weak) IBOutlet UILabel *videoSize;
@property (nonatomic, weak) IBOutlet UIProgressView *recordProgress;

@end


@implementation ZYRecordVedioViewController
...
- (void)compressVideo
{
    ...
    //    NSString *savePath=[cachePath stringByAppendingPathComponent:@"myMovie.mov"];
    //    NSURL *saveUrl=[NSURL fileURLWithPath:savePath];
    
    // 通过文件的 url 获取到这个文件的资源
    AVURLAsset *avAsset = [[AVURLAsset alloc] initWithURL:vedioSavePath options:nil];
        ...
        [exportSession exportAsynchronouslyWithCompletionHandler:^{
            ...
            if ([exportSession status] == AVAssetExportSessionStatusCompleted) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    _recordVedioSuccessBlock(fileData);
...
//        }
//        NSLog(@"outputUrl98765432:%@",outputFileURL);
